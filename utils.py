...
class CTCLabelConverter(object):
    ...
    def decode(self, preds, length):
        ...
        print(f"Original preds shape: {preds.shape}")
        if preds.dim() == 3:  # (sequence_length, batch_size, num_classes)
            preds = preds.permute(1, 0, 2)  # to (batch_size, sequence_length, num_classes)
            print(f"Permuted preds shape: {preds.shape}")
        
        preds = preds.argmax(dim=2)  # max along the channel dimension
        print(f"After argmax preds shape: {preds.shape}")
        
        preds = preds.permute(1, 0).contiguous()  # permute to (sequence_length, batch_size)
        print(f"After permute preds shape: {preds.shape}")

        texts = []
        for pred, l in zip(preds.permute(1, 0), length):
            t = pred[:l]  # take only the valid length part
            char_list = []
            prev_char_idx = -1
            for i in range(l):
                char_idx = t[i].item()
                if char_idx != 0 and char_idx != prev_char_idx:  # remove duplicate and blank
